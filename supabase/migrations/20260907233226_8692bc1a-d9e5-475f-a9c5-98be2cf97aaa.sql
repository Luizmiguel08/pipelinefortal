CREATE OR REPLACE FUNCTION public.atividade_eventos(p_dias integer DEFAULT 7, p_corretor uuid DEFAULT NULL::uuid, p_limit integer DEFAULT 100)
 RETURNS TABLE(id uuid, created_at timestamp with time zone, lead_id uuid, lead_nome text, corretor_id uuid, corretor_nome text, de text, para text, automatico boolean)
 LANGUAGE sql
 STABLE
...
AS $function$
  SELECT e.id, e.created_at, l.id, l.nome, c.id, c.nome,
         ...
         (e.origem IS DISTINCT FROM 'manual') AS automatico
...
CREATE OR REPLACE FUNCTION public.atividade_corretores(p_dias integer DEFAULT 7)
 RETURNS TABLE(corretor_id uuid, nome text, ativo boolean, movimentacoes bigint, manuais bigint, automaticas bigint, hoje bigint, ultima_movimentacao timestamp with time zone, ultima_edicao timestamp with time zone, leads_total bigint, leads_qualificados bigint)
 LANGUAGE sql
 STABLE
 SET search_path TO 'public'
AS $function$
  ...
  ev AS (
    SELECT l.corretor_id,
           ...
           count(*) FILTER (WHERE e.origem IS NOT DISTINCT FROM 'manual')::bigint AS manuais,
           count(*) FILTER (WHERE e.origem IS DISTINCT FROM 'manual')::bigint AS automaticas,
